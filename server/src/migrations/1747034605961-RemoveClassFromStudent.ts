...
export class RemoveClassFromStudent1747034605961 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `, ["event_management_nra","VIEW","student_by_year","SELECT `student_class`.`year` AS `year`, `student_class`.`studentReferenceId` AS `studentReferenceId`, `student`.`tz` AS `studentTz`, `student`.`name` AS `studentName`, CONCAT(`student_class`.`studentReferenceId`, '_', `student_class`.`year`) AS `id`, GROUP_CONCAT(DISTINCT `class`.`name` SEPARATOR ', ') AS `class_names`, GROUP_CONCAT(DISTINCT `class`.`id` SEPARATOR ', ') AS `class_reference_ids` FROM `student_classes` `student_class` LEFT JOIN `classes` `class` ON `class`.`id` = `student_class`.`classReferenceId`  LEFT JOIN `students` `student` ON `student`.`id` = `student_class`.`studentReferenceId` GROUP BY `student_class`.`studentReferenceId`, `student_class`.`year`"]);
    }
