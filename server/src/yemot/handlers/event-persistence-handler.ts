...
export class EventPersistenceHandler {
  ...
  private logStart(operation: string): void {
    this.logger.log(`Starting EventPersistenceHandler.${operation}`);
  }
  ...
  private logComplete(operation: string, result?: any): void {
    if (result) {
      this.logger.log(`Completed EventPersistenceHandler.${operation}: ${JSON.stringify(result)}`);
    } else {
      this.logger.log(`Completed EventPersistenceHandler.${operation}`);
    }
  }
  ...
  private logError(operation: string, error: Error): void {
    this.logger.error(`Error in EventPersistenceHandler.${operation}: ${error.message}`, error.stack);
  }
  ...
  async saveEvent(
   ...
   existingEvent?: DBEvent | null
    ...
    if (eventDate) this.logger.log(`Event date: ${eventDate}`);
    ...
    try {
      ...
      if (path) {
        ...
        this.logger.log(`Setting path: ${path.id} - ${path.name}`);
      }
  ...
  async addEventNote(event: DBEvent, note: string): Promise<EventNote> {
    ...
    try {
      ...
      this.logComplete('addEventNote', { eventId: event.id, noteId: savedNote.id });
      return savedNote;
    } catch (error) {
  ...
  async findStudentEvents(student: Student, includeCompleted?: boolean): Promise<DBEvent[]> {
    ...
    try {
      let queryBuilder = this.dataSource.getRepository(DBEvent)
        ...
        .where('event.studentReferenceId = :studentId', { studentId: student.id });
      
      if (includeCompleted === false) {
        queryBuilder = queryBuilder.andWhere('event.completed = :completed', { completed: false });
      } else if (includeCompleted === true) {
        queryBuilder = queryBuilder.andWhere('event.completed = :completed', { completed: true });
      }
      
      const events = await queryBuilder.getMany();
      this.logComplete('findStudentEvents', { studentId: student.id, count: events.length });
      return events;
    } catch (error) {
