  ...
  private async authenticateStudent(): Promise<boolean> {
    ...
    try {
      ...
      const student = await this.withRetry(
        async () => {
          const tz = await this.call.readDigits(this.call.getText('AUTHENTICATION.ID_PROMPT'), {
        ...
        this.call.getText('AUTHENTICATION.STUDENT_NOT_FOUND'),
      ...
      await this.call.playMessage(this.call.getText('GENERAL.WELCOME', { name: student.name }));
  ...
  private async presentMainMenu(): Promise<MenuOption> {
    ...
    try {
      ...
      const selection = await this.withRetry(
        async () => {
          let menuPrompt = this.call.getText('MENU.MAIN_MENU_BASE');
          const allowedOptions = [
            MenuOption.EVENT_REPORTING,
            // MenuOption.EXIT // Exit is usually handled by hangup or a generic "press 9 to exit"
          ];

          menuPrompt += ` ${this.call.getText('MENU.MENU_OPTIONS.EVENT_REPORTING', { option: MenuOption.EVENT_REPORTING })}`;

          if (this.hasEventsForPathSelection) {
            menuPrompt += ` ${this.call.getText('MENU.MENU_OPTIONS.PATH_SELECTION', { option: MenuOption.PATH_SELECTION })}`;
            allowedOptions.push(MenuOption.PATH_SELECTION);
          }

          if (this.hasEventsForVoucherSelection) {
            menuPrompt += ` ${this.call.getText('MENU.MENU_OPTIONS.VOUCHER_SELECTION', { option: MenuOption.VOUCHER_SELECTION })}`;
            allowedOptions.push(MenuOption.VOUCHER_SELECTION);
          }

          if (this.hasEventsForUpdate) {
            menuPrompt += ` ${this.call.getText('MENU.MENU_OPTIONS.POST_EVENT_UPDATE', { option: MenuOption.POST_EVENT_UPDATE })}`;
            allowedOptions.push(MenuOption.POST_EVENT_UPDATE);
          }
          // menuPrompt += ` ${this.call.getText('MENU.MENU_OPTIONS.EXIT', { option: MenuOption.EXIT})}`; // Consider if EXIT should always be an option here
      ...
      switch (this.selectedMenuOption) {
        case MenuOption.EVENT_REPORTING:
          confirmationMessage = this.call.getText('MENU.MENU_CONFIRMATIONS.EVENT_REPORTING');
          break;
        case MenuOption.PATH_SELECTION:
          confirmationMessage = this.call.getText('MENU.MENU_CONFIRMATIONS.PATH_SELECTION');
          break;
        case MenuOption.VOUCHER_SELECTION:
          confirmationMessage = this.call.getText('MENU.MENU_CONFIRMATIONS.VOUCHER_SELECTION');
          break;
        case MenuOption.POST_EVENT_UPDATE:
          confirmationMessage = this.call.getText('MENU.MENU_CONFIRMATIONS.POST_EVENT_UPDATE');
          break;
        case MenuOption.EXIT:
          confirmationMessage = this.call.getText('MENU.MENU_CONFIRMATIONS.EXIT');
          break;
      }
