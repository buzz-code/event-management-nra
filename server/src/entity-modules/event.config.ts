    ...
    exporter: {
      ...
      getExportHeaders(): IHeader[] {
        return [
          ...
          { value: 'student.motherName', label: 'שם האם' },
          { value: 'student.motherContact', label: 'טלפון האם' },
          { value: 'student.fatherName', label: 'שם האב' },
          { value: 'student.fatherContact', label: 'טלפון האב' },
          { value: 'student.motherPreviousName', label: 'שם משפחה קודם של האם' },
          { value: (row: Event) => row.notes.map(note => note.noteText).join('\r\n'), label: 'הערות' },
